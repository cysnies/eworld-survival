...
public class RdnRmnSdTTzSyHandler implements IHandler {
   public RdnRmnSdTTzSyHandler() {
      super();
   }

   public Span handle(List tokens, Options options) {
      int month = ((RepeaterMonthName.MonthName)((RepeaterMonthName)((Token)tokens.get(1)).getTag(RepeaterMonthName.class)).getType()).ordinal();
      int day = (Integer)((ScalarDay)((Token)tokens.get(2)).getTag(ScalarDay.class)).getType();
      int year = (Integer)((ScalarYear)((Token)tokens.get(5)).getTag(ScalarYear.class)).getType();
