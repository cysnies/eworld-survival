...
public class CBXNmsBlock_147 extends NmsBlock {
   ...
   public CBXNmsBlock_147(int type, int data, TileEntityBlock tileEntityBlock) {
      super(type, data);
      this.nbtData = (NBTTagCompound)fromNative(tileEntityBlock.getNbtData());
   }

   public CBXNmsBlock_147(int type, int data, NBTTagCompound nbtData) {
      super(type, data);
   ...
   public static CBXNmsBlock_147 get(World world, Vector position, int type, int data) {
      if (!hasTileEntity(type)) {
         ...
         if (te != null) {
            ...
            te.b(tag);
            return new CBXNmsBlock_147(type, data, tag);
         } else {
            return null;
         }
      }
   ...
   public static boolean set(World world, Vector position, BaseBlock block) {
      ...
      if (!hasTileEntity(world.getBlockTypeIdAt(position.getBlockX(), position.getBlockY(), position.getBlockZ()))) {
         ...
         if (block instanceof CBXNmsBlock_147) {
            CBXNmsBlock_147 nmsProxyBlock = (CBXNmsBlock_147)block;
            data = nmsProxyBlock.getNmsData(position);
         } else if (block instanceof TileEntityBlock) {
            CBXNmsBlock_147 nmsProxyBlock = new CBXNmsBlock_147(block.getId(), block.getData(), block);
            data = nmsProxyBlock.getNmsData(position);
         }
   ...
   public static net.minecraft.server.v1_4_R1.Block getNmsBlock(int type) {
      return type >= 0 && type < net.minecraft.server.v1_4_R1.Block.byId.length ? net.minecraft.server.v1_4_R1.Block.byId[type] : null;
   }
   ...
   public static boolean isValidBlockType(int type) throws NoClassDefFoundError {
      return type == 0 || type >= 1 && type < net.minecraft.server.v1_4_R1.Block.byId.length && net.minecraft.server.v1_4_R1.Block.byId[type] != null;
   }
