...
public class CBXNmsBlock_161 extends NmsBlock {
   ...
   public CBXNmsBlock_161(int type, int data, TileEntityBlock tileEntityBlock) {
      super(type, data);
      this.nbtData = (NBTTagCompound)fromNative(tileEntityBlock.getNbtData());
   }

   public CBXNmsBlock_161(int type, int data, NBTTagCompound nbtData) {
      super(type, data);
   ...
   public static CBXNmsBlock_161 get(World world, Vector position, int type, int data) {
      if (!hasTileEntity(type)) {
         ...
         if (te != null) {
            ...
            te.b(tag);
            return new CBXNmsBlock_161(type, data, tag);
         } else {
            return null;
         }
      }
   ...
   public static boolean set(World world, Vector position, BaseBlock block) {
      ...
      if (!hasTileEntity(world.getBlockTypeIdAt(position.getBlockX(), position.getBlockY(), position.getBlockZ()))) {
         ...
         if (block instanceof CBXNmsBlock_161) {
            CBXNmsBlock_161 nmsProxyBlock = (CBXNmsBlock_161)block;
            data = nmsProxyBlock.getNmsData(position);
         } else if (block instanceof TileEntityBlock) {
            CBXNmsBlock_161 nmsProxyBlock = new CBXNmsBlock_161(block.getId(), block.getData(), block);
            data = nmsProxyBlock.getNmsData(position);
         }
   ...
   public static net.minecraft.server.v1_6_R1.Block getNmsBlock(int type) {
      return type >= 0 && type < net.minecraft.server.v1_6_R1.Block.byId.length ? net.minecraft.server.v1_6_R1.Block.byId[type] : null;
   }
   ...
   public static boolean isValidBlockType(int type) throws NoClassDefFoundError {
      return type == 0 || type >= 1 && type < net.minecraft.server.v1_6_R1.Block.byId.length && net.minecraft.server.v1_6_R1.Block.byId[type] != null;
   }
