...
public class CBXNmsBlock_prePackage extends NmsBlock {
   private static final Logger logger = Logger.getLogger(CBXNmsBlock_prePackage.class.getCanonicalName());
   ...
   public CBXNmsBlock_prePackage(int type, int data, TileEntityBlock tileEntityBlock) {
      super(type, data);
      this.nbtData = (NBTTagCompound)fromNative(tileEntityBlock.getNbtData());
   }

   public CBXNmsBlock_prePackage(int type, int data, NBTTagCompound nbtData) {
      super(type, data);
   ...
   public static CBXNmsBlock_prePackage get(World world, Vector position, int type, int data) {
      if (!hasTileEntity(type)) {
         ...
         if (te != null) {
            ...
            te.b(tag);
            return new CBXNmsBlock_prePackage(type, data, tag);
         } else {
            return null;
         }
      }
   ...
   public static boolean set(World world, Vector position, BaseBlock block) {
      ...
      if (!hasTileEntity(world.getBlockTypeIdAt(position.getBlockX(), position.getBlockY(), position.getBlockZ()))) {
         ...
         if (block instanceof CBXNmsBlock_prePackage) {
            CBXNmsBlock_prePackage nmsProxyBlock = (CBXNmsBlock_prePackage)block;
            data = nmsProxyBlock.getNmsData(position);
         } else if (block instanceof TileEntityBlock) {
            CBXNmsBlock_prePackage nmsProxyBlock = new CBXNmsBlock_prePackage(block.getId(), block.getData(), block);
            data = nmsProxyBlock.getNmsData(position);
         }
   ...
   public static boolean hasTileEntity(int type) {
      Block nmsBlock = getNmsBlock(type);
      if (nmsBlock == null) {
         return false;
      } else {
         try {
   ...
   public static Block getNmsBlock(int type) {
      return type >= 0 && type < Block.byId.length ? Block.byId[type] : null;
   }
