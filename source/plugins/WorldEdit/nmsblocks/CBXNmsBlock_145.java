...
public class CBXNmsBlock_145 extends NmsBlock {
   private static final Logger logger = Logger.getLogger(CBXNmsBlock_145.class.getCanonicalName());
   ...
   public CBXNmsBlock_145(int type, int data, TileEntityBlock tileEntityBlock) {
      super(type, data);
      this.nbtData = (NBTTagCompound)fromNative(tileEntityBlock.getNbtData());
   }

   public CBXNmsBlock_145(int type, int data, NBTTagCompound nbtData) {
      super(type, data);
   ...
   public static CBXNmsBlock_145 get(World world, Vector position, int type, int data) {
      if (!hasTileEntity(type)) {
         ...
         if (te != null) {
            ...
            te.b(tag);
            return new CBXNmsBlock_145(type, data, tag);
         } else {
            return null;
         }
      }
   ...
   public static boolean set(World world, Vector position, BaseBlock block) {
      ...
      if (!hasTileEntity(world.getBlockTypeIdAt(position.getBlockX(), position.getBlockY(), position.getBlockZ()))) {
         ...
         if (block instanceof CBXNmsBlock_145) {
            CBXNmsBlock_145 nmsProxyBlock = (CBXNmsBlock_145)block;
            data = nmsProxyBlock.getNmsData(position);
         } else if (block instanceof TileEntityBlock) {
            CBXNmsBlock_145 nmsProxyBlock = new CBXNmsBlock_145(block.getId(), block.getData(), block);
            data = nmsProxyBlock.getNmsData(position);
         }
