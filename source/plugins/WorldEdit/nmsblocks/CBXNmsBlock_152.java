...
public class CBXNmsBlock_152 extends NmsBlock {
   ...
   public CBXNmsBlock_152(int type, int data, TileEntityBlock tileEntityBlock) {
      super(type, data);
      this.nbtData = (NBTTagCompound)fromNative(tileEntityBlock.getNbtData());
   }

   public CBXNmsBlock_152(int type, int data, NBTTagCompound nbtData) {
      super(type, data);
   ...
   public static CBXNmsBlock_152 get(World world, Vector position, int type, int data) {
      if (!hasTileEntity(type)) {
         ...
         if (te != null) {
            ...
            te.b(tag);
            return new CBXNmsBlock_152(type, data, tag);
         } else {
            return null;
         }
      }
   ...
   public static boolean set(World world, Vector position, BaseBlock block) {
      ...
      if (!hasTileEntity(world.getBlockTypeIdAt(position.getBlockX(), position.getBlockY(), position.getBlockZ()))) {
         ...
         if (block instanceof CBXNmsBlock_152) {
            CBXNmsBlock_152 nmsProxyBlock = (CBXNmsBlock_152)block;
            data = nmsProxyBlock.getNmsData(position);
         } else if (block instanceof TileEntityBlock) {
            CBXNmsBlock_152 nmsProxyBlock = new CBXNmsBlock_152(block.getId(), block.getData(), block);
            data = nmsProxyBlock.getNmsData(position);
         }
   ...
   public static net.minecraft.server.v1_5_R2.Block getNmsBlock(int type) {
      return type >= 0 && type < net.minecraft.server.v1_5_R2.Block.byId.length ? net.minecraft.server.v1_5_R2.Block.byId[type] : null;
   }
   ...
   public static boolean isValidBlockType(int type) throws NoClassDefFoundError {
      return type == 0 || type >= 1 && type < net.minecraft.server.v1_5_R2.Block.byId.length && net.minecraft.server.v1_5_R2.Block.byId[type] != null;
   }
