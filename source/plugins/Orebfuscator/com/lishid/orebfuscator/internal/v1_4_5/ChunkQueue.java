...
import net.minecraft.server.v1_4_5.Packet56MapChunkBulk;
...
public class ChunkQueue extends LinkedList implements IChunkQueue {
   ...
   public boolean isEmpty() {
      if (this.player.getHandle().netServerHandler.disconnected) {
         this.internalQueue.clear();
   ...
   public void FinishedProcessing(IPacket56 packet) {
      if (this.lastPacket != null) {
         this.player.getHandle().netServerHandler.sendPacket(this.lastPacket);
         this.lastPacket = null;
      }
   ...
   private void processInput() {
      if (this.processingQueue.isEmpty() && !this.internalQueue.isEmpty()) {
         NetworkManager networkManager = (NetworkManager)this.player.getHandle().netServerHandler.networkManager;
