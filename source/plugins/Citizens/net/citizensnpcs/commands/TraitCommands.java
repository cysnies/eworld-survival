...
public class TraitCommands {
   ...
   public void add(CommandContext args, CommandSender sender, NPC npc) throws CommandException {
      ...
      if (added.size() > 0) {
         Messaging.sendTr(sender, "citizens.commands.trait.added", Joiner.on(", ").join(added));
      }

      if (failed.size() > 0) {
         Messaging.sendTr(sender, "citizens.commands.trait.failed-to-add", Joiner.on(", ").join(failed));
      }

   }

   private void addTrait(NPC npc, Class clazz, CommandSender sender) {
      npc.addTrait(clazz);
   ...
   public void configure(CommandContext args, CommandSender sender, NPC npc) throws CommandException {
      ...
      if (!sender.hasPermission("citizens.npc.trait-configure." + traitName) && !sender.hasPermission("citizens.npc.trait-configure.*")) {
         ...
         } else if (!CommandConfigurable.class.isAssignableFrom(clazz)) {
   ...
   public void remove(CommandContext args, CommandSender sender, NPC npc) throws CommandException {
      ...
      if (removed.size() > 0) {
         Messaging.sendTr(sender, "citizens.commands.trait.removed", Joiner.on(", ").join(removed));
      }

      if (failed.size() > 0) {
         Messaging.sendTr(sender, "citizens.commands.trait.failed-to-remove", Joiner.on(", ").join(failed));
      }
   ...
   public void toggle(CommandContext args, CommandSender sender, NPC npc) throws CommandException {
      ...
      if (failed.size() > 0) {
         Messaging.sendTr(sender, "citizens.commands.trait.failed-to-change", Joiner.on(", ").join(failed));
      }
