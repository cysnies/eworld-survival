...
public class EditorCommands {
   ...
   @Requirements(
      selected = true,
      ownership = true,
      excludedTypes = {EntityType.BLAZE, EntityType.ENDER_DRAGON, EntityType.GHAST, EntityType.BAT, EntityType.WITHER, EntityType.SQUID}
   )
   public void path(CommandContext args, Player player, NPC npc) {
      Editor.enterOrLeave(player, ((Waypoints)npc.getTrait(Waypoints.class)).getEditor(player, args));
   }
   ...
   public void text(CommandContext args, Player player, NPC npc) {
      Editor.enterOrLeave(player, ((Text)npc.getTrait(Text.class)).getEditor(player));
   }
}
