...
public class MinecraftBlockExaminer implements BlockExaminer {
   ...
   private static final Set PASSABLE;
   private static final Set UNWALKABLE;
   ...
   public float getCost(BlockSource source, PathPoint point) {
      ...
      if (above != Material.WEB && in != Material.WEB) {
         if (below != Material.SOUL_SAND && below != Material.ICE) {
            return this.isLiquid(above, below, in) ? 0.5F : 0.5F;
         } else {
            return 1.0F;
         }
      } else {
   ...
   public static boolean canStandOn(Block block) {
      ...
      return canStandOn(block.getType()) && canStandIn(up.getType()) && canStandIn(up.getRelative(BlockFace.UP).getType());
   }
   ...
   static {
      PASSABLE = EnumSet.of(Material.AIR, Material.DEAD_BUSH, Material.DETECTOR_RAIL, Material.DIODE, Material.DIODE_BLOCK_OFF, Material.DIODE_BLOCK_ON, Material.FENCE_GATE, Material.ITEM_FRAME, Material.LADDER, Material.LEVER, Material.LONG_GRASS, Material.CARPET, Material.MELON_STEM, Material.NETHER_FENCE, Material.PUMPKIN_STEM, Material.POWERED_RAIL, Material.RAILS, Material.RED_ROSE, Material.RED_MUSHROOM, Material.REDSTONE, Material.REDSTONE_TORCH_OFF, Material.REDSTONE_TORCH_OFF, Material.REDSTONE_WIRE, Material.SIGN, Material.SIGN_POST, Material.SNOW, Material.STRING, Material.STONE_BUTTON, Material.SUGAR_CANE_BLOCK, Material.TRIPWIRE, Material.VINE, Material.WALL_SIGN, Material.WHEAT, Material.WATER, Material.WEB, Material.WOOD_BUTTON, Material.WOODEN_DOOR, Material.STATIONARY_WATER);
      UNWALKABLE = EnumSet.of(Material.AIR, Material.LAVA, Material.STATIONARY_LAVA, Material.CACTUS);
      UP = new Vector(0, 1, 0);
   }
}
