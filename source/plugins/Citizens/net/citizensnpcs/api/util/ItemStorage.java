...
public class ItemStorage {
   ...
   private static Iterable deserialiseColors(DataKey key) {
   ...
   private static Map deserialiseEnchantments(DataKey root, ItemStack res) {
   ...
   private static FireworkEffect deserialiseFireworkEffect(DataKey key) {
      ...
      effect.with(Type.valueOf(key.getString("type")));
   ...
   private static void deserialiseMeta(DataKey root, ItemStack res) {
      ...
      if (root.keyExists("firework")) {
         FireworkMeta meta = (FireworkMeta)ensureMeta(res);
      ...
      if (root.keyExists("book")) {
         BookMeta meta = (BookMeta)ensureMeta(res);
   ...
   private static ItemMeta ensureMeta(ItemStack stack) {
   ...
   public static ItemStack loadItemStack(DataKey root) {
      ...
      if (raw != null && raw.length() != 0) {
         ...
         if (id <= 0) {
            ...
            ItemStack res = new ItemStack(id, root.getInt("amount"), (short)root.getInt("durability", root.getInt("data", 0)));
   ...
   private static void serialiseColors(DataKey key, List colors) {
   ...
   private static void serialiseEnchantments(DataKey key, Map enchantments) {
      for(Map.Entry enchantment : enchantments.entrySet()) {
         key.setInt(Integer.toString(((Enchantment)enchantment.getKey()).getId()), (Integer)enchantment.getValue());
      }
