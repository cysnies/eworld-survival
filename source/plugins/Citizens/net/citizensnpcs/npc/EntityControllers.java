...
public class EntityControllers {
   ...
   public static EntityController createForType(EntityType type) {
      Class<? extends EntityController> controllerClass = (Class)TYPES.get(type);
      if (controllerClass == null) {
         throw new IllegalArgumentException("Unknown EntityType: " + type);
      } else {
         try {
            return (EntityController)controllerClass.newInstance();
         } catch (Throwable ex) {
   ...
   public static void setEntityControllerForType(EntityType type, Class controller) {
