...
public class Hunger implements Effect {
   ...
   private void checkHunger(RealDamageEvent e, Player p, int data) {
      if (this.r.nextInt(10000) < 1200) {
         LivingEntity le = e.getVictim();
         PotionEffect effect = new PotionEffect(PotionEffectType.HUNGER, data * 20, 3, false);
         le.addPotionEffect(effect, true);
         p.sendMessage(this.get(245));

         try {
            if (le instanceof Player) {
               ((Player)le).sendMessage(this.get(250));
            }
