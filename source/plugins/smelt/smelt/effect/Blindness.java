...
public class Blindness implements Effect {
   private static final int ID = 4;
   private Random r = new Random();
   private String pn;

   public Blindness(Main main) {
      super();
      this.pn = main.getPn();
   }
   ...
   private void checkBlindness(RealDamageEvent e, Player p, int data) {
      if (this.r.nextInt(10000) < 1000) {
         ...
         PotionEffect effect = new PotionEffect(PotionEffectType.BLINDNESS, data * 20, 0, false);
         le.addPotionEffect(effect, true);
         p.sendMessage(this.get(220));

         try {
            if (le instanceof Player) {
               ((Player)le).sendMessage(this.get(225));
            }
