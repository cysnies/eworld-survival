...
public class Confusion implements Effect {
   ...
   private void checkConfusion(RealDamageEvent e, Player p, int data) {
      if (this.r.nextInt(10000) < 1000) {
         LivingEntity le = e.getVictim();
         PotionEffect effect = new PotionEffect(PotionEffectType.CONFUSION, data * 20, 0, false);
         le.addPotionEffect(effect, true);
         p.sendMessage(this.get(235));

         try {
            if (le instanceof Player) {
               ((Player)le).sendMessage(this.get(240));
            }
