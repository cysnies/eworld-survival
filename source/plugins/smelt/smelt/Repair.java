...
public class Repair implements Listener {
   ...
   public void onInventoryClick(InventoryClickEvent e) {
      if (e.getInventory().getType().equals(InventoryType.ANVIL) && e.getSlot() == 2) {
         ...
         if (is1 == null || is1.getTypeId() == 0) {
            return;
         }

         int bad0 = this.getBad(is0);
         int bad1 = this.getBad(is1);
         if (bad0 < this.defaultUse && bad1 < this.defaultUse) {
            e.setCancelled(true);

            for(HumanEntity he : e.getViewers()) {
               if (he instanceof Player) {
                  Player p = (Player)he;
                  p.sendMessage(UtilFormat.format(this.pn, "fail", new Object[]{this.get(620)}));
               }
            }
