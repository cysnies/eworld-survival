...
public class InfoCommand extends MultiverseCommand {
   ...
   public void runCommand(CommandSender sender, List args) {
      ...
      if (args.size() == 0) {
         ...
         if (this.worldManager.isMVWorld((String)args.get(0))) {
            ...
            if (this.worldManager.getUnloadedWorlds().contains(args.get(0))) {
            ...
            try {
               pageNum = Integer.parseInt((String)args.get(0)) - 1;
         ...
         try {
            pageNum = Integer.parseInt((String)args.get(1)) - 1;
   ...
   private List buildEntireCommand(MultiverseWorld world, Player p) {
      ...
      message.add(new FancyMessage("Spawn Location: ", this.plugin.getLocationManipulation().strCoords(spawn), colors));
      message.add(new FancyMessage("World Scale: ", world.getScaling() + "", colors));
      if (world.getPrice() > (double)0.0F) {
         String formattedAmount;
         if (world.getCurrency() <= 0 && this.plugin.getVaultHandler().getEconomy() != null) {
            formattedAmount = this.plugin.getVaultHandler().getEconomy().format(world.getPrice());
         } else {
   ...
   private static String toCommaSeperated(List list) {
      if (list != null && list.size() != 0) {
         if (list.size() == 1) {
            ...
            result = result + " and " + (String)list.get(list.size() - 1);
            return result;
         }
      } else {
         return "";
   ...
   private static void showPage(int page, CommandSender sender, List doc) {
      ...
      if (sender instanceof Player) {
         for(FancyText fancyT : (List)doc.get(page)) {
