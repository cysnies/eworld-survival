...
public class ClassFileWriter {
      ...
      if (this.itsCurrentMethod == null) {
         ...
         if (attrLength > 65536) {
            ...
            if (stackMapTableLength > 0) {
               ...
               stackMap.write(codeAttribute, index);
            }
   ...
   public void addPush(double k) {
      if (k == (double)0.0F) {
         this.add(14);
         if ((double)1.0F / k < (double)0.0F) {
            this.add(119);
         }
      } else if (k != (double)1.0F && k != (double)-1.0F) {
         this.addLoadConstant(k);
      } else {
         this.add(15);
         if (k < (double)0.0F) {
      ...
      if (0 <= jumpTarget && jumpTarget <= this.itsCodeBufferTop) {
         if (caseIndex < -1) {
            ...
            if (0 <= switchStart && switchStart <= this.itsCodeBufferTop - 16 - padSize - 1) {
               ...
               } else if (0 <= caseOffset && caseOffset + 4 <= this.itsCodeBufferTop) {
   ...
   private int getWriteSize() {
      ...
      for(int i = 0; i < this.itsFields.size(); ++i) {
         size += ((ClassFileField)((ClassFileField)this.itsFields.get(i))).getWriteSize();
      }

      size += 2;

      for(int i = 0; i < this.itsMethods.size(); ++i) {
         size += ((ClassFileMethod)((ClassFileMethod)this.itsMethods.get(i))).getWriteSize();
      }
   ...
   public byte[] toByteArray() {
      ...
      for(int i = 0; i < this.itsInterfaces.size(); ++i) {
         int interfaceIndex = (Short)((Short)this.itsInterfaces.get(i));
      ...
      private int execute(int bci) {
         ...
         switch (bc) {
            ...
            case 50:
               ...
               String arrayType = (String)ClassFileWriter.this.itsConstantPool.getConstantData(typeIndex);
            ...
            case 180:
               this.pop();
            case 178:
               int var37 = this.getOperand(bci + 1, 2);
               FieldOrMethodRef f = (FieldOrMethodRef)ClassFileWriter.this.itsConstantPool.getConstantData(var37);
            ...
            case 184:
            case 185:
               int var36 = this.getOperand(bci + 1, 2);
               FieldOrMethodRef m = (FieldOrMethodRef)ClassFileWriter.this.itsConstantPool.getConstantData(var36);
            ...
            case 188:
               ...
               int indexx = ClassFileWriter.this.itsConstantPool.addClass("[" + componentType);
               this.push(TypeInfo.OBJECT((short)indexx));
               break;
            case 189:
