...
class CodePrinter {
   ...
   private int put_string_literal_char(int pos, int c, boolean in_string) {
      ...
      if (backslash_symbol) {
         ...
         this.buffer[pos] = '\\';
         this.buffer[pos + 1] = 'u';
         this.buffer[pos + 2] = digit_to_hex_letter(15 & c >> 12);
         this.buffer[pos + 3] = digit_to_hex_letter(15 & c >> 8);
         this.buffer[pos + 4] = digit_to_hex_letter(15 & c >> 4);
         this.buffer[pos + 5] = digit_to_hex_letter(15 & c);
         pos += 6;
      }
   ...
   public void indent(int level) {
      ...
      int indent_end;
      for(indent_end = pos + indent_size; pos != tab_end; ++pos) {
         this.buffer[pos] = '\t';
      }
