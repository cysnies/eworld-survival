...
class Menubar extends JMenuBar implements ActionListener {
   ...
   private List interruptOnlyItems = Collections.synchronizedList(new ArrayList());
   private List runOnlyItems = Collections.synchronizedList(new ArrayList());
   ...
   public void updateEnabled(boolean interrupted) {
      for(int i = 0; i != this.interruptOnlyItems.size(); ++i) {
         JMenuItem item = (JMenuItem)this.interruptOnlyItems.get(i);
         item.setEnabled(interrupted);
      }

      for(int i = 0; i != this.runOnlyItems.size(); ++i) {
         JMenuItem item = (JMenuItem)this.runOnlyItems.get(i);
         item.setEnabled(!interrupted);
      }
