...
public class SwingGui extends JFrame implements GuiCallback {
   ...
   private final Map toplevels = Collections.synchronizedMap(new HashMap());
   private final Map fileWindows = Collections.synchronizedMap(new HashMap());
   ...
   public void setVisible(boolean b) {
      ...
      if (b) {
         ...
         this.context.split.setDividerLocation((double)0.5F);
   ...
   private void updateEnabled(boolean interrupted) {
      ...
      for(int cc = this.toolBar.getComponentCount(); ci < cc; ++ci) {
      ...
      if (!cmd.equals("Cut") && !cmd.equals("Copy") && !cmd.equals("Paste")) {
         if (cmd.equals("Step Over")) {
            ...
            if (cmd.equals("Go to function...")) {
               ...
               int cols;
               int rows = cols = (int)Math.sqrt((double)count);
               if (rows * cols < count) {
                  ++cols;
                  if (rows * cols < count) {
                     ++rows;
               ...
               Object obj = this.getFileWindow(cmd);
               if (obj != null) {
