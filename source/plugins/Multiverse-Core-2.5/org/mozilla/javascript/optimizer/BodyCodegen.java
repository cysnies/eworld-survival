...
class BodyCodegen {
   ...
   private void generateGenerator() {
      this.cfw.startMethod(this.codegen.getBodyMethodName(this.scriptOrFn), this.codegen.getBodyMethodSignature(this.scriptOrFn), (short)10);
      this.initBodyGeneration();
      short var10003 = this.firstFreeLocal;
      this.firstFreeLocal = (short)(var10003 + 1);
      this.argsLocal = var10003;
   ...
   private void generatePrologue() {
      ...
      this.localsMax = this.firstFreeLocal;
      if (this.isGenerator) {
         var10003 = this.firstFreeLocal;
         this.firstFreeLocal = (short)(var10003 + 1);
         this.operationLocal = var10003;
         this.localsMax = this.firstFreeLocal;
         this.cfw.addALoad(this.thisObjLocal);
         var10003 = this.firstFreeLocal;
         this.firstFreeLocal = (short)(var10003 + 1);
         this.generatorStateLocal = var10003;
         this.localsMax = this.firstFreeLocal;
            ...
            for(Node n : this.finallys.keySet()) {
               if (n.getType() == 125) {
                  ...
                  for(int i = 0; i < ret.jsrPoints.size(); ++i) {
                     ...
                     this.cfw.add(167, (Integer)ret.jsrPoints.get(i));
                     ++c;
                  }
               }
            }
   ...
   private void generateStatement(Node node) {
      ...
      switch (type) {
         ...
         case 60:
            ...
            this.addScriptRuntimeInvoke("enumInit", "(Ljava/lang/Object;Lorg/mozilla/javascript/Context;I)Ljava/lang/Object;");
         ...
         case 125:
            if (this.isGenerator) {
               ...
               int finallyRegister = this.getNewWordLocal();
         ...
         case 141:
            ...
            int local = this.getNewWordLocal();
            if (this.isGenerator) {
               this.cfw.add(1);
               this.cfw.addAStore(local);
            }
   ...
   private void visitObjectLiteral(Node node, Node child, boolean topLevel) {
      Object[] properties = node.getProp(12);
   ...
   private void generateCatchBlock(int exceptionType, short savedVariableObject, int catchLabel, int exceptionLocal, int handler) {
      ...
      this.cfw.addALoad(savedVariableObject);
      this.cfw.addAStore(this.variableObjectLocal);
      this.exceptionTypeToName(exceptionType);
      ...
      if (trueGOTO != -1 && falseGOTO != -1) {
         ...
         if (type != 53 && type != 52) {
            ...
            if (childNumberFlag != -1) {
               ...
               String routine = type != 14 && type != 16 ? "cmp_LE" : "cmp_LT";
   ...
   private void visitSetName(Node node, Node child) {
      String name;
      for(name = node.getFirstChild().getString(); child != null; child = child.getNext()) {
         this.generateExpression(child, node);
      }
   ...
   private void visitGetProp(Node node, Node child) {
      ...
      if (node.getType() == 34) {
         this.cfw.addALoad(this.contextLocal);
         this.addScriptRuntimeInvoke("getObjectPropNoWarn", "(Ljava/lang/Object;Ljava/lang/String;Lorg/mozilla/javascript/Context;)Ljava/lang/Object;");
      } else {
         int childType = child.getType();
   ...
   private short getNewWordPairLocal(boolean isConst) {
      ...
      if (result < 1023) {
         ...
         for(int i = this.firstFreeLocal + 2; i < 1024; ++i) {
            if (this.locals[i] == 0) {
   ...
   private short getNewWordLocal() {
      ...
      for(int i = this.firstFreeLocal + 1; i < 1024; ++i) {
         if (this.locals[i] == 0) {
            this.firstFreeLocal = (short)i;
            if (this.localsMax < this.firstFreeLocal) {
               this.localsMax = this.firstFreeLocal;
   ...
   private short getConsecutiveSlots(int count, boolean isConst) {
      ...
      short result;
      for(result = this.firstFreeLocal; result < 1023; ++result) {
         int i;
   ...
   private void incReferenceWordLocal(short local) {
      int var10002 = this.locals[local]++;
   }
   ...
   static class FinallyReturnPoint {
      public List jsrPoints = new ArrayList();
