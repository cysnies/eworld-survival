...
public class Parser {
   ...
   void addWarning(String messageId, int position, int length) {
      this.addWarning(messageId, (String)null, position, length);
   }
   ...
   void addError(String messageId, int position, int length) {
      this.addError(messageId, (String)null, position, length);
   }
   ...
   String lookupMessage(String messageId) {
      return this.lookupMessage(messageId, (String)null);
   }
   ...
   void reportError(String messageId) {
      this.reportError(messageId, (String)null);
   }
   ...
   void reportError(String messageId, int position, int length) {
      this.reportError(messageId, (String)null, position, length);
   }
   ...
   private int peekToken() throws IOException {
      if (this.currentFlaggedToken != 0) {
         ...
         boolean sawEOL;
         for(sawEOL = false; tt == 1 || tt == 161; tt = this.ts.getToken()) {
            if (tt == 1) {
               ++lineno;
               sawEOL = true;
   ...
   private AstRoot parse() throws IOException {
      ...
      if (this.syntaxErrorCount != 0) {
         ...
         if (!this.compilerEnv.isIdeMode()) {
            throw this.errorReporter.runtimeError(msg, this.sourceURI, baseLineno, (String)null, 0);
         }
      }
   ...
   private AstNode statements() throws IOException {
      return this.statements((AstNode)null);
   }
   ...
   private AstNode statementHelper() throws IOException {
      ...
      switch (tt) {
         ...
         case 153:
            ...
            if (!(pn instanceof VariableDeclaration) || this.peekToken() != 82) {
               return pn;
            }
            break;
         case 160:
   ...
   private TryStatement tryStatement() throws IOException {
      ...
      if (peek != 124) {
         ...
         for(; this.matchToken(124); clauses.add(catchNode)) {
   ...
   private AstNode letStatement() throws IOException {
      ...
      if (this.peekToken() == 87) {
         ...
         pn = this.variables(153, pos, true);
      }
   ...
   private AstNode expr() throws IOException {
      ...
      int opPos;
      for(int pos = pn.getPosition(); this.matchToken(89); pn = new InfixExpression(89, pn, this.assignExpr(), opPos)) {
         opPos = this.ts.tokenBeg;
         if (this.compilerEnv.isStrictMode() && !pn.hasSideEffects()) {
   ...
   private AstNode bitOrExpr() throws IOException {
      AstNode pn;
      int opPos;
      for(pn = this.bitXorExpr(); this.matchToken(9); pn = new InfixExpression(9, pn, this.bitXorExpr(), opPos)) {
         opPos = this.ts.tokenBeg;
      }
   ...
   private AstNode bitXorExpr() throws IOException {
      AstNode pn;
      int opPos;
      for(pn = this.bitAndExpr(); this.matchToken(10); pn = new InfixExpression(10, pn, this.bitAndExpr(), opPos)) {
         opPos = this.ts.tokenBeg;
      }
   ...
   private AstNode bitAndExpr() throws IOException {
      AstNode pn;
      int opPos;
      for(pn = this.eqExpr(); this.matchToken(11); pn = new InfixExpression(11, pn, this.eqExpr(), opPos)) {
         opPos = this.ts.tokenBeg;
      }
   ...
   private AstNode unaryExpr() throws IOException {
      ...
      switch (tt) {
         ...
         case 21:
            this.consumeToken();
            AstNode var10 = new UnaryExpression(28, this.ts.tokenBeg, this.unaryExpr());
            ((AstNode)var10).setLineno(line);
            return var10;
         case 22:
            this.consumeToken();
            AstNode var9 = new UnaryExpression(29, this.ts.tokenBeg, this.unaryExpr());
            ((AstNode)var9).setLineno(line);
            return var9;
         case 26:
         case 27:
         case 32:
         ...
         while(true) {
            switch (tt) {
               case 145:
                  ...
                  int beg = this.ts.tokenBeg;
                  AstNode expr = (AstNode)(this.peekToken() == 86 ? new EmptyExpression(beg, this.ts.tokenEnd - beg) : this.expr());
   ...
   private List argumentList() throws IOException {
      if (this.matchToken(88)) {
         return null;
      } else {
      ...
      while(true) {
         ...
         switch (tt) {
            case 83:
               ...
               int rb = -1;
               int var19 = this.ts.lineno;
               AstNode expr = this.expr();
               int var20 = this.getNodeEnd(expr);
               if (this.mustMatchToken(84, "msg.no.bracket.index")) {
                  rb = this.ts.tokenBeg;
                  var20 = this.ts.tokenEnd;
               }

               ElementGet g = new ElementGet(pos, var20 - pos);
               g.setTarget(pn);
               g.setElement(expr);
               g.setParens(lb, rb);
               g.setLineno(var19);
   ...
   private AstNode propertyAccess(int tt, AstNode pn) throws IOException {
      ...
      if (!this.compilerEnv.isXmlAvailable()) {
         ...
         label57: {
            ...
            InfixExpression result = (InfixExpression)(xml ? new XmlMemberGet() : new PropertyGet());
            if (xml && tt == 108) {
               result.setType(108);
            }
   ...
   private AstNode attributeAccess() throws IOException {
      ...
      switch (tt) {
         ...
         case 83:
            return this.xmlElemRef(atPos, (Name)null, -1);
         default:
      ...
      if (this.matchToken(144)) {
         ...
         switch (this.nextToken()) {
            ...
            case 39:
               ...
               break;
            case 83:
               return this.xmlElemRef(atPos, name, colonPos);
            default:
   ...
   private AstNode primaryExpr() throws IOException {
      ...
      switch (tt) {
         ...
         case 44:
         case 45:
            int s = this.ts.tokenBeg;
            int end = this.ts.tokenEnd;
            return new KeywordLiteral(s, end - s, tt);
         case 83:
   ...
   private AstNode parenExpr() throws IOException {
      ...
      try {
         ...
         if (this.peekToken() != 119) {
            ...
            pn.setLineno(lineno);
            ParenthesizedExpression var7 = pn;
            return var7;
         }

         pn = this.generatorExpression(e, begin);
      } finally {
         this.inForInit = wasInForInit;
      }
   ...
   private AstNode name(int ttFlagged, int tt) throws IOException {
      ...
      if (0 != (ttFlagged & 131072) && this.peekToken() == 103) {
         ...
         return (AstNode)(this.compilerEnv.isXmlAvailable() ? this.propertyName(-1, nameString, 0) : this.createNameNode(true, 39));
      }
   }
   ...
   protected void checkActivationName(String name, int token) {
      if (this.insideFunction()) {
         ...
         if (!"arguments".equals(name) && (this.compilerEnv.getActivationNames() == null || !this.compilerEnv.getActivationNames().contains(name))) {
            if ("length".equals(name) && token == 33 && this.compilerEnv.getLanguageVersion() == 120) {
               activation = true;
            }
         } else {
            activation = true;
   ...
   Node destructuringAssignmentHelper(int variableType, Node left, Node right, String tempName) {
      ...
      if (empty) {
         comma.addChildToBack(this.createNumber((double)0.0F));
      }
   ...
   boolean destructuringArray(ArrayLiteral array, int variableType, String tempName, Node parent, List destructuringNames) {
      boolean empty = true;
      ...
      for(AstNode n : array.getElements()) {
         if (n.getType() == 128) {
            ...
            if (n.getType() == 39) {
               ...
               parent.addChildToBack(new Node(setOp, this.createName(49, name, (Node)null), rightElem));
               if (variableType != -1) {
   ...
   boolean destructuringObject(ObjectLiteral node, int variableType, String tempName, Node parent, List destructuringNames) {
      boolean empty = true;
      int setOp = variableType == 154 ? 155 : 8;
