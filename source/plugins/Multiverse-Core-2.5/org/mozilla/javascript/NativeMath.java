...
final class NativeMath extends IdScriptableObject {
      ...
      if (!f.hasTag(MATH_TAG)) {
         ...
         switch (methodId) {
            ...
            case 2:
               ...
               x = x == (double)0.0F ? (double)0.0F : (x < (double)0.0F ? -x : x);
               break;
            case 3:
            case 4:
               x = ScriptRuntime.toNumber(args, 0);
               if (x == x && (double)-1.0F <= x && x <= (double)1.0F) {
            ...
            case 8:
               x = ScriptRuntime.toNumber(args, 0);
               x = x != Double.POSITIVE_INFINITY && x != Double.NEGATIVE_INFINITY ? Math.cos(x) : Double.NaN;
               break;
            case 9:
               x = ScriptRuntime.toNumber(args, 0);
               x = x == Double.POSITIVE_INFINITY ? x : (x == Double.NEGATIVE_INFINITY ? (double)0.0F : Math.exp(x));
               break;
            case 10:
               x = ScriptRuntime.toNumber(args, 0);
               x = Math.floor(x);
            ...
            case 11:
               x = ScriptRuntime.toNumber(args, 0);
               x = x < (double)0.0F ? Double.NaN : Math.log(x);
               break;
            case 12:
            case 13:
            ...
            case 17:
               x = ScriptRuntime.toNumber(args, 0);
               x = x != Double.POSITIVE_INFINITY && x != Double.NEGATIVE_INFINITY ? Math.sin(x) : Double.NaN;
               break;
            case 18:
               x = ScriptRuntime.toNumber(args, 0);
               x = Math.sqrt(x);
   ...
   private double js_pow(double x, double y) {
      ...
      if (y != y) {
         ...
         if ((double)1.0F / x > (double)0.0F) {
            result = y > (double)0.0F ? (double)0.0F : Double.POSITIVE_INFINITY;
         } else {
            long y_long = (long)y;
            if ((double)y_long == y && (y_long & 1L) != 0L) {
               result = y > (double)0.0F ? (double)-0.0F : Double.NEGATIVE_INFINITY;
            } else {
               result = y > (double)0.0F ? (double)0.0F : Double.POSITIVE_INFINITY;
            }
         }
         ...
         if (result != result) {
            if (y == Double.POSITIVE_INFINITY) {
               if (!(x < (double)-1.0F) && !((double)1.0F < x)) {
                  if ((double)-1.0F < x && x < (double)1.0F) {
                     result = (double)0.0F;
                  }
               } else {
               ...
               result = y > (double)0.0F ? Double.POSITIVE_INFINITY : (double)0.0F;
            } else if (x == Double.NEGATIVE_INFINITY) {
               long y_long = (long)y;
               if ((double)y_long == y && (y_long & 1L) != 0L) {
                  result = y > (double)0.0F ? Double.NEGATIVE_INFINITY : (double)-0.0F;
               } else {
                  result = y > (double)0.0F ? Double.POSITIVE_INFINITY : (double)0.0F;
               }
            }
