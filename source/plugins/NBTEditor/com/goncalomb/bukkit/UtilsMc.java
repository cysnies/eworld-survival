...
public final class UtilsMc {
   private static HashSet NON_SOLID_BLOCK_IDS = new HashSet();

   static {
      NON_SOLID_BLOCK_IDS.add((byte)0);

      Material[] var3;
      for(Material mat : var3 = Material.values()) {
   ...
   public static Location airLocation(Location loc) {
      ...
      for(int maxY = world.getMaxHeight(); y < maxY && !NON_SOLID_BLOCK_IDS.contains((byte)world.getBlockAt(x, y, z).getTypeId()); ++y) {
      }

      return new Location(world, (double)x + (double)0.5F, (double)y + 0.2, (double)z + (double)0.5F);
   }
   ...
   public static Vector faceToDelta(BlockFace face) {
      return (new Vector(1, 1, 1)).add(new Vector(face.getModX(), face.getModY(), face.getModZ())).multiply((double)0.5F);
   }

   public static Vector faceToDelta(BlockFace face, double distance) {
      Vector delta = faceToDelta(face);
      return (new Vector((double)-0.5F, (double)-0.5F, (double)-0.5F)).add(delta).normalize().multiply(distance).add(delta);
   }
   ...
   public static ItemStack newItem(MaterialData material, String name) {
      return newItem((MaterialData)material, name, (String[])null);
   }
