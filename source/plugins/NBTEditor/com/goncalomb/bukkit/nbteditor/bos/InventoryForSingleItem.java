...
public abstract class InventoryForSingleItem extends IInventoryForBos {
   public InventoryForSingleItem(String title, HashMap placeholders, ItemStack item, BookOfSouls bos, Player owner) {
   ...
   protected ItemStack getItemToCheck(InventoryClickEvent event) {
      ...
      if (isShift && slot > 8 && event.getInventory().firstEmpty() == 4) {
         ...
         return slot == 4 && !isShift && event.getCursor().getType() != Material.AIR ? event.getCursor() : null;
      }
   }
