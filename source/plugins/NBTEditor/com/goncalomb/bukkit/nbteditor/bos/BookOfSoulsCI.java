   ...
   public void onLeftClick(PlayerInteractEvent event, PlayerDetails details) {
      ...
      if (bos == null) {
         ...
         if (event.getAction() == Action.LEFT_CLICK_BLOCK) {
            ...
            if (bos.getEntityNBT() instanceof MinecartContainerNBT && block.getType() == Material.CHEST) {
               if (event.getPlayer().isSneaking()) {
            ...
            if (event.getPlayer().isSneaking() && bos.getEntityNBT() instanceof FallingBlockNBT) {
            ...
            location = event.getClickedBlock().getLocation().add(UtilsMc.faceToDelta(event.getBlockFace(), (double)0.5F));
         } else {
            Block block = UtilsMc.getTargetBlock(player);
            if (block.getType() != Material.AIR) {
               location = UtilsMc.airLocation(block.getLocation()).add((double)0.0F, (double)0.5F, (double)0.0F);
            }
         }
