...
public class Base64 {
      ...
      if (source == null) {
         ...
         if (len == 0) {
            ...
            for(int var13 = off; var13 < off + len; ++var13) {
               sbiDecode = DECODABET[source[var13] & 255];
               if (sbiDecode < -5) {
                  throw new IOException(String.format("Bad Base64 input character decimal %d in array position %d", source[var13] & 255, var13));
               }
   ...
   public static String encodeFromFile(String filename) throws IOException {
      ...
      try {
         File file = new File(filename);
         byte[] buffer = new byte[Math.max((int)((double)file.length() * 1.4 + (double)1.0F), 40)];
         int length = 0;
         int numBytes = 0;
