...
public class TCChunkGenerator extends ChunkGenerator {
   ...
   private ArrayList BlockPopulator = new ArrayList();
         ...
         for(int _x = 0; _x < 16; ++_x) {
            for(int _z = 0; _z < 16; ++_z) {
               for(int y = 0; y < max_y; ++y) {
                  byte block = BlockArray[_x << this.heightBitsPlusFour | _z << this.heightBits | y];
                  if (block != 0) {
                     int sectionId = y >> 4;
                     ...
                     SectionBlocks[sectionId][(y & 15) << 8 | _z << 4 | _x] = block;
                  }
               }
            }
         }
