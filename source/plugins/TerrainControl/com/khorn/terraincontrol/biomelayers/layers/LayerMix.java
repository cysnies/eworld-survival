   ...
   public int[] GetBiomes(int cacheId, int paramInt1, int paramInt2, int paramInt3, int paramInt4) {
      ...
      for(int i = 0; i < paramInt4; ++i) {
         for(int j = 0; j < paramInt3; ++j) {
            ...
            int cachedId;
            if ((currentPiece & 256) != 0) {
               cachedId = currentPiece & 255;
            } else if (this.worldConfig.FrozenOcean && (currentPiece & 512) != 0) {
               cachedId = DefaultBiome.FROZEN_OCEAN.Id;
            } else {
               cachedId = DefaultBiome.OCEAN.Id;
            }

            if (this.worldConfig.RiversEnabled && (currentPiece & 3072) != 0 && !this.worldConfig.biomeConfigs[cachedId].RiverBiome.isEmpty()) {
