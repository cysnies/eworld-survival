...
public class ChunkProviderTC {
   ...
   private void generateTerrain(int chunkX, int chunkZ, byte[] paramArrayOfByte) {
      ...
      this.u = this.GenerateTerrainNoise(this.u, chunkX * i1, 0, chunkZ * i1, i4, i5, i6);
      this.BiomeArray = this.localWorld.getBiomes(this.BiomeArray, chunkX * 16, chunkZ * 16, ChunkMaxX, ChunkMaxZ);
      double d1 = (double)0.125F;
      double d10 = (double)0.25F;
      int z_step = 1 << this.heightBits;
      double d15 = (double)0.25F;
               ...
               for(int piece_y = 0; piece_y < 8; ++piece_y) {
                  ...
                  for(int piece_x = 0; piece_x < 4; ++piece_x) {
                     ...
                     for(int piece_z = 0; piece_z < 4; ++piece_z) {
                        int biomeId = this.BiomeArray[(z * 4 + piece_z) * 16 + piece_x + x * 4];
                        int i15 = 0;
   ...
   boolean ReplaceForBiomeAndReturnWaterless(int paramInt1, int paramInt2, byte[] paramArrayOfByte) {
      int dryBlock = 256;
      double d1 = (double)0.03125F;
      this.v = this.r.Noise3D(this.v, paramInt1 * 16, paramInt2 * 16, 0, 16, 16, 1, d1 * (double)2.0F, d1 * (double)2.0F, d1 * (double)2.0F);
      ...
      for(int x = 0; x < 16; ++x) {
         for(int z = 0; z < 16; ++z) {
            ...
            int biomeId = this.BiomeArray[z + x * 16];
            int stone_noise = (int)(this.v[x + z * 16] / (double)3.0F + (double)3.0F + this.rnd.nextDouble() * (double)0.25F);
            int i5 = -1;
            ...
            for(int y = this.heightMinusOne; y >= 0; --y) {
               ...
               if (y < 5 && this.worldSettings.createAdminium(y) && y <= this.rnd.nextInt(5)) {
   ...
   private double[] GenerateTerrainNoise(double[] outArray, int paramInt1, int paramInt2, int paramInt3, int max_X, int max_Y, int max_Z) {
      ...
      double d1 = 684.412 * this.worldSettings.getFractureHorizontal();
      double d2 = 684.412 * this.worldSettings.getFractureVertical();
      if (this.worldSettings.oldTerrainGenerator) {
         this.j = this.a.Noise2D(this.j, paramInt1, paramInt3, max_X, max_Z, 1.121, 1.121);
      }

      this.k = this.b.Noise2D(this.k, paramInt1, paramInt3, max_X, max_Z, (double)200.0F, (double)200.0F);
      this.g = this.q.Noise3D(this.g, paramInt1, paramInt2, paramInt3, max_X, max_Y, max_Z, d1 / (double)80.0F, d2 / (double)160.0F, d1 / (double)80.0F);
      ...
      for(int x = 0; x < max_X; ++x) {
         for(int z = 0; z < max_Z; ++z) {
            ...
            BiomeConfig biomeConfig = this.worldSettings.biomeConfigs[biomeId];
            double d3 = this.k[i2D] / (double)8000.0F;
            if (d3 < (double)0.0F) {
               d3 = -d3 * 0.3;
            }
            ...
            for(int y = 0; y < max_Y; ++y) {
               double d8 = (this.HeightFactor - (double)y) * (double)12.0F * (double)128.0F / (double)this.height / this.VolatilityFactor;
               if (d8 > (double)0.0F) {
                  d8 *= (double)4.0F;
               }

               double d9 = this.h[i3D] / (double)512.0F * biomeConfig.volatility1;
               double d10 = this.i[i3D] / (double)512.0F * biomeConfig.volatility2;
               double d11 = (this.g[i3D] / (double)10.0F + (double)1.0F) / (double)2.0F;
               double d7;
               ...
               if (!biomeConfig.disableNotchHeightControl) {
                  d7 += d8;
                  if (y > max_Y - 4) {
                     double d12 = (double)((float)(y - (max_Y - 4)) / 3.0F);
                     d7 = d7 * ((double)1.0F - d12) + (double)-10.0F * d12;
                  }
               }
   ...
   private void oldTerrainNoise(int x, int z, int i4, int max_X, int max_Y, double d3) {
      if (this.worldSettings.biomeMode == TerrainControl.getBiomeModeManager().OLD_GENERATOR) {
         this.VolatilityFactor = (double)1.0F - this.localWorld.getBiomeFactorForOldBM(z * 48 + 17 + x * 3);
      } else {
         int biomeId = this.BiomeArray[x + 2 + (z + 2) * (max_X + 5)];
         this.VolatilityFactor = (double)1.0F - (double)(this.worldSettings.biomeConfigs[biomeId].BiomeTemperature * this.worldSettings.biomeConfigs[biomeId].BiomeWetness);
      }

      this.VolatilityFactor *= this.VolatilityFactor;
      this.VolatilityFactor = (double)1.0F - this.VolatilityFactor * this.VolatilityFactor;
      this.VolatilityFactor = (this.g[i4] + (double)256.0F) / (double)512.0F * this.VolatilityFactor;
      if (this.VolatilityFactor > (double)1.0F) {
         this.VolatilityFactor = (double)1.0F;
      }

      if (this.VolatilityFactor < (double)0.0F || d3 < (double)0.0F) {
         this.VolatilityFactor = (double)0.0F;
      }

      this.VolatilityFactor += (double)0.5F;
      this.HeightFactor = (double)max_Y * ((double)2.0F + d3) / (double)4.0F;
   }
   ...
   private void newTerrainNoise(int x, int z, int max_X, int max_Y, double d3) {
      ...
      double d4 = (double)f3;
      this.VolatilityFactor = (double)f2;
      d4 += d3 * 0.2;
      this.HeightFactor = (double)max_Y * ((double)2.0F + d4) / (double)4.0F;
   }

   public byte[] generate(int x, int z) {
      this.rnd.setSeed((long)x * 341873128712L + (long)z * 132897987541L);
      byte[] arrayOfByte = new byte[ChunkMaxX * this.height * ChunkMaxZ];
      this.generateTerrain(x, z, arrayOfByte);
      boolean dry = this.ReplaceForBiomeAndReturnWaterless(x, z, arrayOfByte);
