...
public class AuthMeSpoutListener implements Listener {
   ...
   public void onSpoutCraftEnable(SpoutCraftEnableEvent event) {
      if (SpoutCfg.getInstance().getBoolean("LoginScreen.enabled") && this.data.isAuthAvailable(event.getPlayer().getName().toLowerCase()) && !PlayerCache.getInstance().isAuthenticated(event.getPlayer().getName().toLowerCase())) {
