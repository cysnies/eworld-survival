...
public class Response {
   ...
   private Object parseString(boolean parseAtoms, boolean returnString) {
      ...
      if (b == 34) {
         ...
         int copyto;
         for(copyto = this.index; this.index < this.size && (b = this.buffer[this.index]) != 34; ++this.index) {
            if (b == 92) {
               ++this.index;
            }
         ...
         if (this.index >= this.size) {
            ...
            ++this.index;
            return returnString ? ASCIIUtility.toString(this.buffer, start, copyto) : new ByteArray(this.buffer, start, copyto - start);
         }
      } else if (b != 123) {
         if (parseAtoms) {
            int start = this.index;
