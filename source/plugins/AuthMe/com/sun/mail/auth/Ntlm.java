...
public class Ntlm {
   ...
   private byte[] makeDesKey(byte[] input, int off) {
      ...
      byte[] out = new byte[8];
      out[0] = (byte)in[off + 0];
      out[1] = (byte)(in[off + 0] << 7 & 255 | in[off + 1] >> 1);
      out[2] = (byte)(in[off + 1] << 6 & 255 | in[off + 2] >> 2);
      out[3] = (byte)(in[off + 2] << 5 & 255 | in[off + 3] >> 3);
      out[4] = (byte)(in[off + 3] << 4 & 255 | in[off + 4] >> 4);
      out[5] = (byte)(in[off + 4] << 3 & 255 | in[off + 5] >> 5);
      out[6] = (byte)(in[off + 5] << 2 & 255 | in[off + 6] >> 6);
      out[7] = (byte)(in[off + 6] << 1 & 255);
      return out;
   }
