...
public class ResultSetMetaData implements java.sql.ResultSetMetaData {
   ...
   public String getColumnCharacterEncoding(int column) throws SQLException {
      ...
      if (mysqlName != null) {
         javaName = CharsetMapping.getJavaEncodingForMysqlEncoding(mysqlName, (Connection)null);
      }
   ...
   public int getPrecision(int column) throws SQLException {
      ...
      if (isDecimalType(f.getSQLType())) {
         return f.getDecimals() > 0 ? clampedGetLength(f) - 1 + f.getPrecisionAdjustFactor() : clampedGetLength(f) + f.getPrecisionAdjustFactor();
      } else {
   ...
   public int getScale(int column) throws SQLException {
      Field f = this.getField(column);
      return isDecimalType(f.getSQLType()) ? f.getDecimals() : 0;
   }
   ...
   public String getTableName(int column) throws SQLException {
      return this.useOldAliasBehavior ? this.getField(column).getTableName() : this.getField(column).getTableNameNoAliases();
   }
