...
public class TimeUtil {
   ...
   private static long jdbcCompliantZoneShift(Calendar sessionCalendar, Calendar targetCalendar, Date dt) {
      ...
      try {
         ...
         var5 = targetCalendar.getTime().getTime();
      } finally {
   ...
   static final java.sql.Date fastDateCreate(int year, int month, int day, Calendar targetCalendar) {
      Calendar dateCal = (Calendar)(targetCalendar == null ? new GregorianCalendar() : targetCalendar);
      dateCal.clear();
      dateCal.set(year, month - 1, day, 0, 0, 0);
      dateCal.set(14, 0);
      long dateAsMillis = 0L;
      ...
      if (hour >= 0 && hour <= 23) {
         if (minute >= 0 && minute <= 59) {
            if (second >= 0 && second <= 59) {
               ...
               } catch (IllegalAccessError var7) {
                  timeAsMillis = cal.getTime().getTime();
               }

               return new Time(timeAsMillis);
            } else {
               throw SQLError.createSQLException("Illegal minute value '" + second + "'" + "' for java.sql.Time type in value '" + timeFormattedString(hour, minute, second) + ".", "S1009");
            }
         } else {
            throw SQLError.createSQLException("Illegal minute value '" + minute + "'" + "' for java.sql.Time type in value '" + timeFormattedString(hour, minute, second) + ".", "S1009");
         }
      } else {
         throw SQLError.createSQLException("Illegal hour value '" + hour + "' for java.sql.Time type in value '" + timeFormattedString(hour, minute, second) + ".", "S1009");
      }
   }
   ...
   static final Time fastTimeCreate(int hour, int minute, int second, Calendar targetCalendar) throws SQLException {
      if (hour >= 0 && hour <= 23) {
         if (minute >= 0 && minute <= 59) {
            if (second >= 0 && second <= 59) {
               Calendar cal = (Calendar)(targetCalendar == null ? new GregorianCalendar() : targetCalendar);
               cal.clear();
               cal.set(1970, 0, 1, hour, minute, second);
               long timeAsMillis = 0L;
            ...
            if (canonicalTz == null) {
               ...
               if (abbreviatedTimezone != null) {
                  if (abbreviatedTimezone.length != 1) {
                     ...
                     throw SQLError.createSQLException(Messages.getString("TimeUtil.TooGenericTimezoneId", new Object[]{timezoneStr, possibleTimezones}), "01S00");
                  }
