...
public class Liveness {
         ...
         if (tb.exit == null) {
            ...
            if (changed) {
               ...
               for(int i = 0; i < tb.exit.length; ++i) {
                  ...
                  if (e.status != 2) {
                     for(int k = 0; k < n; ++k) {
                        if (!tb.inputs[k] && usage[k] == 0 && e.inputs[k]) {
                           tb.inputs[k] = true;
                           changed = true;
                        }
                     }
   ...
   private boolean computeLiveness2except(TypedBlock tb) {
      ...
      boolean changed;
      for(changed = false; handlers != null; handlers = handlers.next) {
         TypedBlock h = (TypedBlock)handlers.body;
   ...
   private void computeUsage(CodeIterator ci, TypedBlock[] blocks, int maxLocals) throws BadBytecode {
      for(TypedBlock tb : blocks) {
         this.localsUsage = tb.localsUsage = new byte[maxLocals];
         int pos = tb.position;
