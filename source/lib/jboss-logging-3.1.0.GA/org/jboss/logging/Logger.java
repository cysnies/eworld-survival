...
public abstract class Logger implements Serializable, BasicLogger {
   ...
   public static Object getMessageLogger(Class type, String category) {
      return getMessageLogger(type, category, Locale.getDefault());
   }

   public static Object getMessageLogger(Class type, String category, Locale locale) {
      ...
      if (loggerClass == null && country != null && country.length() > 0) {
         try {
            loggerClass = Class.forName(join(type.getName(), "$logger", language, country, (String)null), true, type.getClassLoader()).asSubclass(type);
      ...
      if (loggerClass == null && language != null && language.length() > 0) {
         try {
            loggerClass = Class.forName(join(type.getName(), "$logger", language, (String)null, (String)null), true, type.getClassLoader()).asSubclass(type);
      ...
      if (loggerClass == null) {
         try {
            loggerClass = Class.forName(join(type.getName(), "$logger", (String)null, (String)null, (String)null), true, type.getClassLoader()).asSubclass(type);
